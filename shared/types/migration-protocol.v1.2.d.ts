...
export type MarketingRole = 'hook' | 'pain_amplify' | 'product_demo' | 'demo' | 'cta' | 'social_proof' | 'brand_story' | string;
export type EmotionVibe = 'urgent' | 'warm' | 'tech' | 'humorous' | 'inspiring' | 'trustworthy' | string;
export type MatchStatus = 'matched' | 'gap' | 'pending';
export type TransitionPresentation = 'cut' | 'fade' | 'slide' | 'wipe' | 'flip' | 'clock_wipe';
export type TransitionTimingType = 'linear' | 'spring';
export type TransitionDirection = 'from-left' | 'from-right' | 'from-top' | 'from-bottom';
export type TransitionOverlayType = 'none' | 'light_leak' | 'flash' | 'color_wash';
export type SequenceLayout = 'fill' | 'none';
export type VisualMotionPreset = 'static' | 'zoom_in' | 'push_in' | 'pan' | 'shake';
...
//# sourceMappingURL=migration-protocol.v1.2.d.ts.map