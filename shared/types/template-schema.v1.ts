...
import type {
  CapabilityLayerPlan,
  ContentDomain,
  VisualPhenomenonMechanism,
} from './director-grounding.v1.js'
...
export type TemplateTransitionPresentation =
  | 'cut'
  | 'fade'
  | 'slide'
  | 'wipe'
...
export type TemplateTransitionDirection =
  | 'from-left'
  | 'from-right'
  | 'from-top'
  | 'from-bottom'

export type TemplateTransitionOverlayType =
  | 'none'
  | 'light_leak'
  | 'flash'
  | 'color_wash'
...
export type TemplateVisualMotionPreset =
  | 'static'
  | 'zoom_in'
  | 'push_in'
  | 'pan'
