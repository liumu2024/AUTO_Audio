...
export interface AgentTraceArtifactRef {
    ...
    category?: 'model_input' | 'model_raw_output' | 'model_structured_output' | 'api_raw_io' | 'tool_output' | 'audit' | 'render_input' | 'render_output' | 'summary' | 'debug';
    bytes?: number;
}
...
export interface AgentTraceManifestV1 {
    ...
    latest_event?: Pick<AgentTraceEventV1, 'seq' | 'timestamp' | 'phase' | 'actor' | 'event' | 'status' | 'summary'>;
}
export interface AgentTraceIndexV1 {
    ...
    artifacts_by_phase: Partial<Record<AgentTracePhase, Partial<Record<NonNullable<AgentTraceArtifactRef['category']>, AgentTraceArtifactRef[]>>>>;
}
//# sourceMappingURL=agent-trace.v1.d.ts.map