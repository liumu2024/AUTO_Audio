...
import type {
  DirectorActionRecord,
  DirectorRecoverableError,
...
function trimLedger(ledger: DirectorActionRecord[]) {
  return ledger.slice(-20)
}

function maybePlanSteps(steps: DirectorPlanStepRun[]) {
  return steps.length ? steps : undefined
}

function createPlanStepRuns(action: DirectorAction): DirectorPlanStepRun[] {
...
function markFirstPlannedStepRunning(
  steps: DirectorPlanStepRun[] | undefined,
  startedAt: string,
): DirectorPlanStepRun[] | undefined {
  if (!steps?.length) return steps
  let didMark = false
  return steps.map((step) => {
    if (didMark || step.status !== 'planned') return step
    didMark = true
    return {
      ...step,
      status: 'running',
...
function findStepIndexByTool(
  steps: DirectorPlanStepRun[],
  tools: DirectorToolName[],
): number {
  return steps.findIndex((step) => tools.includes(step.tool))
}

function failedStepIndexForError(
  steps: DirectorPlanStepRun[],
...
function markStepsFailed(
  steps: DirectorPlanStepRun[] | undefined,
  error: DirectorToolError,
  actionType: DirectorActionType | undefined,
  completedAt: string,
): DirectorPlanStepRun[] | undefined {
  if (!steps?.length) return steps
...
export function classifyDirectorFailure(
  message: string,
  actionType?: DirectorActionType,
): DirectorFailureCode {
  ...
  const knownCodes: DirectorFailureCode[] = [
...
export function directorToolErrorFromMessage(
  message: string,
  actionType?: DirectorActionType,
): DirectorToolError {
...
export function recoverableErrorFromMessage(
  message: string,
  actionType?: DirectorActionType,
): DirectorRecoverableError {
  const code = classifyDirectorFailure(message, actionType)
  const suggestionsByCode: Record<DirectorFailureCode, DirectorRecoverableError['suggestions']> = {
    ARK_FILE_QUOTA_EXCEEDED: [
      {
...
export function syncDirectorSessionSnapshot(
  previous: DirectorSessionState | undefined,
...
export function recordDirectorActionPlanned(input: {
  ...
  const record: DirectorActionRecord = {
    id: id('act'),
...
export function recordDirectorActionRunning(input: {
  ...
  if (!lastAction) return input.state
  ...
  return {
    ...input.state,
    lastAction: record,
    actionLedger: trimLedger(
      input.state.actionLedger.map((item) => (item.id === record.id ? record : item)),
    ),
  }
}
...
export function recordDirectorActionCompleted(input: {
  ...
  const record: DirectorActionRecord | undefined = lastAction
    ? {
        ...lastAction,
        status: 'completed',
        phaseAfter,
...
export function recordDirectorActionFailed(input: {
  ...
  const record: DirectorActionRecord | undefined = lastAction
    ? {
        ...
        phaseAfter: 'failed',
        error: input.error,
        planSteps: markStepsFailed(
          lastAction.planSteps,
          toolError,
          input.actionType,
          completedAt,
        ),
