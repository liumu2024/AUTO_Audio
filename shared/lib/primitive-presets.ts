...
] as const satisfies readonly SceneEffects['preset'][]

export type PrimitivePresetId = (typeof PRIMITIVE_PRESET_IDS)[number]

const PRIMITIVE_PRESET_SET = new Set<string>(PRIMITIVE_PRESET_IDS)

export function isPrimitivePreset(
  preset: SceneEffects['preset'] | undefined,
): preset is PrimitivePresetId {
...
export function isLegacyCompositePreset(preset: string | undefined): boolean {
  if (!preset) return false
  return (LEGACY_COMPOSITE_PRESET_IDS as readonly string[]).includes(preset)
}
