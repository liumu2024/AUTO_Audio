...
export const SLOT_TAG_LABELS: Record<string, string> = {
  intro: '开篇',
  opening: '开场',
  water_scene: '水景',
  landscape: '风光',
...
export function creativeRoleLabel(value: string | undefined): string {
  if (!value) return '未判断'
  return CREATIVE_ROLE_LABELS[value] ?? value
}

export function slotTagLabel(value: string): string {
