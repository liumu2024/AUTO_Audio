...
import type { TimelineClip, TimelineProject } from '../types/timeline-protocol.js'

const DEFAULT_TRACKS: TimelineProject['tracks'] = [
...
export function buildTimelineFromStructure(
  ...
  for (const anchor of structure.semantic_anchors) {
    ...
    const displayRole =
      anchor.logic_intent.creative_role ?? anchor.logic_intent.marketing_role
    const roleLabel = creativeRoleLabel(displayRole)
    const roleDisplay =
      roleLabel !== '未判断' ? roleLabel : anchor.anchor_id
    const videoLabel =
      anchor.match.asset_name ?? (isGap ? `[缺口] ${roleDisplay}` : roleDisplay)
...
export function buildOutlineFromStructure(
  structure: MigrationProtocolV12,
): OutlineSegment[] {
...
export function formatOutlineDuration(start: number, end: number): string {
  const fmt = (s: number) => {
