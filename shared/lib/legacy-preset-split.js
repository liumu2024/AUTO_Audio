function childLayer(parent, idSuffix, input) {
    return {
        ...parent,
...
function textureGradeLayer(parent, suffix, input, reason) {
...
function vignetteLayer(parent, vignette, reason) {
...
function grainLayer(parent, grain, reason) {
...
function letterboxLayer(parent, letterbox, reason) {
...
export function splitLegacyColorPortalLayer(layer) {
    if (layer.effects.preset !== 'color_portal_spotlight')
        return [layer];
...
export function splitLegacyKineticColorRippleLayer(layer) {
    if (layer.effects.preset !== 'kinetic_color_ripple')
        return [layer];
    const effect = layer.effects;
    const colorTransform = childLayer(layer, 'color_transform', {
        layerKind: 'color_transform',
...
export function splitLegacyCinematicGradePackLayer(layer) {
    if (layer.effects.preset !== 'cinematic_grade_pack')
        return [layer];
    const effect = layer.effects;
    const layers = [
        textureGradeLayer(layer, 'texture_grade', { base_filter: effect.base_filter, color_grade: effect.color_grade }, 'Split from legacy cinematic_grade_pack: texture grade.'),
    ];
    if (effect.bloom?.enabled) {
        layers.push(childLayer(layer, 'bloom', {
            layerKind: 'texture_grade',
            plugin_id: 'cinematic_texture_grade',
    ...
    if (effect.chromatic_aberration?.enabled) {
        layers.push(childLayer(layer, 'chroma', {
            layerKind: 'texture_grade',
            plugin_id: 'cinematic_texture_grade',
    ...
    if (effect.letterbox?.enabled) {
        layers.push(letterboxLayer(layer, effect.letterbox, 'Split from legacy cinematic_grade_pack: letterbox.'));
    }
    return layers;
}
export function splitLegacyCinematicLightSweepLayer(layer) {
    if (layer.effects.preset !== 'cinematic_light_sweep')
        return [layer];
    const effect = layer.effects;
    const layers = [
        textureGradeLayer(layer, 'texture_grade', { base_filter: effect.base_filter, color_grade: effect.color_grade }, 'Split from legacy cinematic_light_sweep: texture grade.'),
    ...
    if (effect.letterbox?.enabled) {
        layers.push(letterboxLayer(layer, effect.letterbox, 'Split from legacy cinematic_light_sweep: letterbox.'));
    }
    return layers;
}
export function splitLegacyAudioReactiveLayer(layer) {
    if (layer.effects.preset !== 'audio_reactive_cut_driver')
        return [layer];
    const effect = layer.effects;
    const layers = [];
    if (effect.base_filter) {
        layers.push(textureGradeLayer(layer, 'base_filter', { base_filter: effect.base_filter }, 'Split from legacy audio_reactive_cut_driver: base filter.'));
    }
    layers.push(childLayer(layer, 'beat_pulse', {
    ...
    if (effect.flash?.enabled) {
        layers.push(childLayer(layer, 'beat_flash', {
            layerKind: 'audio_driver',
            plugin_id: 'beat_cut_driver',
...
export function splitLegacyMaskSliceLayer(layer) {
    if (layer.effects.preset !== 'mask_slice_transition')
        return [layer];
    const effect = layer.effects;
    const layers = [];
    if (effect.base_filter) {
        layers.push(textureGradeLayer(layer, 'base_filter', { base_filter: effect.base_filter }, 'Split from legacy mask_slice_transition: base filter.'));
    }
    layers.push(childLayer(layer, 'slice_reveal', {
        layerKind: 'mask_reveal',
...
export function splitLegacyRippleDisplacementLayer(layer) {
    if (layer.effects.preset !== 'ripple_displacement')
        return [layer];
    const effect = layer.effects;
    const layers = [];
    if (effect.base_filter) {
        layers.push(textureGradeLayer(layer, 'base_filter', { base_filter: effect.base_filter }, 'Split from legacy ripple_displacement: base filter.'));
    }
    layers.push(childLayer(layer, 'ripple_displacement', {
...
export function splitLegacyEditorialCollageLayer(layer) {
    if (layer.effects.preset !== 'editorial_split_collage')
        return [layer];
    const effect = layer.effects;
    const layers = [
        textureGradeLayer(layer, 'texture_grade', { base_filter: effect.base_filter, color_grade: effect.color_grade }, 'Split from legacy editorial_split_collage: texture grade.'),
    ...
    if (effect.letterbox?.enabled) {
        layers.push(letterboxLayer(layer, effect.letterbox, 'Split from legacy editorial_split_collage: letterbox.'));
    }
    return layers;
}
const SPLITTERS = [
...
export function splitEffectLayer(layer) {
    let current = [layer];
...
export function splitEffectLayers(layers) {
    return layers.flatMap((layer) => splitEffectLayer(layer));
}
/** @deprecated use splitEffectLayer */
export function splitLegacyCompositeLayer(layer) {
    return splitEffectLayer(layer);
}
//# sourceMappingURL=legacy-preset-split.js.map