...
function isRenderableVisualAsset(asset: RenderAsset): boolean {
...
function isMontagePrompt(prompt: string): boolean {
...
function isProductPrompt(prompt: string): boolean {
  const lower = prompt.toLowerCase()
  return (
...
function visualModeForAsset(asset: RenderAsset): VisualLayer['mode'] {
  return asset.type === 'image' ? 'image_motion' : 'material_clip'
}

function sceneDuration(scene: RenderScene): number {
  return Math.max(0.5, scene.end_sec - scene.start_sec)
}

function buildAssetRotationCandidate(plan: RenderPlanV1): RenderPlanV1 | undefined {
  const visualAssets = plan.assets.filter(isRenderableVisualAsset)
  if (visualAssets.length < 2 || plan.scenes.length < 2) return undefined

  const trimCursors = new Map<string, number>()
  return {
    ...clonePlan(plan),
    strategy: 'montage',
...
function buildStabilizedMotionCandidate(plan: RenderPlanV1): RenderPlanV1 | undefined {
  if (!plan.scenes.some((scene) => scene.visual.motion)) return undefined
...
function countVisualSceneCoverage(plan: RenderPlanV1): {
  assetCoverage: number
  uniqueAssetCoverage: number
} {
  const visualAssets = plan.assets.filter(isRenderableVisualAsset)
  ...
  const visualScenes = plan.scenes.filter((scene) => {
    return (
      VISUAL_MODES_REQUIRING_ASSET.has(scene.visual.mode) ||
      Boolean(scene.visual.asset_id)
    )
  })
  if (!visualScenes.length) return { assetCoverage: 1, uniqueAssetCoverage: 1 }

  const referenced = visualScenes
    .map((scene) => scene.visual.asset_id)
    .filter((id): id is string => Boolean(id && visualAssetIds.has(id)))
  ...
  const uniqueAssetCoverage =
    visualAssets.length > 0 ? new Set(referenced).size / expectedUnique : 1
  return { assetCoverage, uniqueAssetCoverage }
}

function effectLayerReadiness(plan: RenderPlanV1): number {
  const layers = plan.scenes.flatMap((scene) => scene.effect_layers ?? [])
  if (!layers.length) return 1
  const ready = layers.filter(
    (layer) => layer.plugin_id && layer.resolution !== 'missing' && layer.effects?.preset,
  ).length
  return ready / layers.length
}

function timelineFit(plan: RenderPlanV1): number {
  if (!plan.scenes.length || plan.duration_sec <= 0) return 0
...
function promptFit(plan: RenderPlanV1, prompt: string): number {
  if (!prompt.trim()) return 0.5
  let score = 0.5
...
function scoreCandidate(input: {
  ...
  const metrics: RenderPlanCandidateMetrics = {
    ...coverage,
    ...
    validationPenalty:
      validation.errors.length * 30 + validation.warnings.length * 3,
  }
  const score =
    (validation.ok ? 60 : 0) +
...
export function selectRenderPlanCandidate(
  input: RenderPlanCandidateInput,
): RenderPlanCandidateSelection {
  const phase = input.phase ?? 'before_save'
  const prompt = input.prompt ?? ''
  const rawCandidates: Array<{
    id: string
    label: string
    plan: RenderPlanV1 | undefined
  }> = [
    {
      id: 'base',
  ...
  const candidates = rawCandidates
    .filter((candidate): candidate is typeof candidate & { plan: RenderPlanV1 } =>
