...
const LAYER_KIND_SET = new Set<string>(CAPABILITY_LAYER_KINDS)
...
function pushError(
  errors: EffectRoadmapValidationError[],
  input: Omit<EffectRoadmapValidationError, 'message'> & { message?: string },
): void {
  errors.push({
...
function validateSegment(
  ...
  for (const atom of atoms) {
    if (!isRecord(atom) || typeof atom.id !== 'string') continue
...
export function validateEffectRoadmap(candidate: unknown): EffectRoadmapValidationResult {
  const errors: EffectRoadmapValidationError[] = []
...
export function assertValidEffectRoadmap(candidate: unknown): EffectRoadmap {
  ...
  return candidate as EffectRoadmap
}
