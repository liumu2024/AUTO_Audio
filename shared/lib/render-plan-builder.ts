...
import type { AssetSegmentV1 } from '../types/asset-analysis.v1.js'
import type { SemanticAnchor, TimelineTransition } from '../types/migration-protocol.v1.2.js'
...
import {
  compileSceneEffectRecipesForAnchor,
  shouldAddAudioReactiveFallback,
  type OverlayCapabilityRecipe,
} from './render-recipe-compiler.js'
import {
...
} from './primitive-effect-detail.js'
...
function mapAssetType(materialType: UserMaterialDto['material_type']): RenderAsset['type'] {
  if (materialType === 'VIDEO') return 'video'
  if (materialType === 'AUDIO') return 'audio'
  return 'image'
}

interface GroundingShotEvent {
  id: string
  start_sec: number
  end_sec: number
...
interface GroundingTransitionObservation {
  id: string
  at_sec: number
  from_shot_id?: string
  to_shot_id?: string
...
type PlanningAnchor = SemanticAnchor & {
  source_anchor_id?: string
  shot_event_id?: string
}

function directorGroundingRecord(structure: MigrationProtocolV12): Record<string, unknown> | null {
...
function readGroundingShotEvents(structure: MigrationProtocolV12): GroundingShotEvent[] {
  const grounding = directorGroundingRecord(structure)
  const shots = grounding?.shot_events
  if (!Array.isArray(shots)) return []
  return shots
    .filter((shot): shot is GroundingShotEvent => {
...
function readGroundingTransitionObservations(
  structure: MigrationProtocolV12,
): GroundingTransitionObservation[] {
  const grounding = directorGroundingRecord(structure)
  const transitions = grounding?.transition_observations
  if (!Array.isArray(transitions)) return []
  return transitions
    .filter((transition): transition is GroundingTransitionObservation => {
...
function findParentAnchorForShot(
  shot: GroundingShotEvent,
  ...
  return (
    anchors.find((anchor) => midpoint >= anchor.start_sec && midpoint <= anchor.end_sec) ??
    anchors.find((anchor) => shot.start_sec < anchor.end_sec && shot.end_sec > anchor.start_sec) ??
    anchors[0]
  )
}

function clampShotTime(value: number, duration: number): number {
...
function buildPlanningAnchors(input: {
...
}): { anchors: PlanningAnchor[]; usingShotEvents: boolean } {
  ...
  const shotAnchors = shots.slice(0, 24).flatMap((shot, index): PlanningAnchor[] => {
    ...
    return [
      {
        ...
        replication_instructions: {
          ...
          visual_generation_prompt:
            shot.visual_summary || parent.replication_instructions.visual_generation_prompt,
          visual_motion: {
            preset:
              parent.replication_instructions.visual_motion?.preset ??
              (shot.camera_motion?.toLowerCase().includes('pan') ? 'pan' : 'static'),
            intensity:
              shot.visual_change_intensity ??
...
function findTransitionObservationForBoundary(input: {
  observations: GroundingTransitionObservation[]
  fromAnchor: PlanningAnchor
  toAnchor: PlanningAnchor
}): GroundingTransitionObservation | undefined {
  const byShotId = input.observations.find(
    (observation) =>
      observation.from_shot_id === input.fromAnchor.shot_event_id &&
      observation.to_shot_id === input.toAnchor.shot_event_id,
  )
  if (byShotId) return byShotId
  return input.observations
    .slice()
    .sort(
      (a, b) =>
        Math.abs(a.at_sec - input.fromAnchor.end_sec) -
        Math.abs(b.at_sec - input.fromAnchor.end_sec),
    )[0]
}

function mapObservedTransition(input: {
  observation: GroundingTransitionObservation | undefined
  fromAnchor: PlanningAnchor
  toAnchor: PlanningAnchor
  index: number
}): TimelineTransition {
  ...
  const durationSec =
    input.observation?.duration_sec && input.observation.duration_sec > 0
  ...
  return {
    ...
    timing: { type: 'linear' },
    ...(isSlide || isWipe ? { direction: 'from-right' as const } : {}),
    overlay: isFlash
      ? {
          type: 'flash',
      ...
      : { type: 'none' },
    reason:
      input.observation?.visual_mechanism ||
...
function buildTransitionsForPlanningAnchors(input: {
  structure: MigrationProtocolV12
  anchors: PlanningAnchor[]
  usingShotEvents: boolean
}): TimelineTransition[] {
  if (!input.usingShotEvents) return input.structure.transitions ?? []
...
function isLandscapeMontageStyle(structure?: MigrationProtocolV12): boolean {
  const recipeText = [
...
function inferStrategy(
  ...
  if (videoCount >= 3) return 'montage'
  if (imageCount >= 4) return 'montage'
  if (videoCount > 0 && imageCount > 0) return 'hybrid'
...
function segmentMatchScore(segment: AssetSegmentV1, role: string): number {
...
function selectMaterialSegment(input: {
  ...
  materials: UserMaterialDto[]
  reservedMaterialIds?: Set<string>
  anchorIndex?: number
  forceSequentialFallback?: boolean
}): { material: UserMaterialDto; segment?: AssetSegmentV1 } | null {
  const {
    anchor,
    materials,
    reservedMaterialIds,
    anchorIndex = 0,
    forceSequentialFallback = false,
  } = input
  ...
  if (exact) {
    const segment = exact.asset_analysis?.segments
      ...
      .sort(
        (a, b) =>
          segmentMatchScore(b, anchor.logic_intent.marketing_role) -
          segmentMatchScore(a, anchor.logic_intent.marketing_role),
      )[0]
  ...
  const candidates = visualMaterials
    ...
    .flatMap((material) =>
    (material.asset_analysis?.segments ?? []).map((segment) => ({
      material,
      segment,
  ...
  const fallbackPool =
    visualMaterials.filter((material) => !reservedMaterialIds?.has(material.id)) ??
    visualMaterials
  const fallback =
    (fallbackPool.length ? fallbackPool : visualMaterials)[
      anchorIndex % Math.max(1, (fallbackPool.length ? fallbackPool : visualMaterials).length)
    ]
...
function buildVisualLayer(
  ...
  const mode: VisualLayer['mode'] = hasMaterial
    ? 'material_clip'
  ...
  return {
    ...
    fit: 'cover',
      motion: {
        preset:
          anchor.replication_instructions.visual_motion?.preset ??
          (selection?.segment?.motion === 'shake'
            ...
            : anchor.logic_intent.marketing_role === 'hook'
              ...
              : 'static'),
        intensity:
          anchor.replication_instructions.visual_motion?.intensity ??
          (anchor.logic_intent.marketing_role === 'hook' ? 0.8 : 0.3),
        easing: anchor.replication_instructions.visual_motion?.easing,
        driver:
          anchor.replication_instructions.visual_motion?.driver ??
          'useCurrentFrame',
      },
    visual_prompt:
      anchor.replication_instructions.visual_generation_prompt ||
...
const COLOR_HINT_PALETTE: ColorHintPaletteEntry[] = [
  {
    token: 'red',
    label: '红',
    color: '#ef4444',
...
function resolvePaletteEntry(input: {
  ...
  evidenceText: string
}): ColorHintPaletteEntry | undefined {
  const explicitToken = firstStringParam(input.params, ['color_token', 'color_name', 'hue'])
  if (explicitToken) {
    const normalized = explicitToken.toLowerCase()
    const explicitMatch = COLOR_HINT_PALETTE.find(
      (item) => item.token === normalized || item.aliases.includes(normalized),
    )
    if (explicitMatch) return explicitMatch
  }

  return COLOR_HINT_PALETTE.find((item) =>
    item.aliases.some((alias) => input.evidenceText.includes(alias)),
  )
}

function resolveColorHintCue(
  anchor: MigrationProtocolV12['semantic_anchors'][number],
  params: Record<string, unknown>,
...
function buildOverlayFromCapabilityRecipe(
  anchor: MigrationProtocolV12['semantic_anchors'][number],
  recipe: OverlayCapabilityRecipe,
): OverlayLayer {
  ...
  const position =
    ...
    params.position === 'right'
      ...
      : 'bottom'
  const fontWeight =
    params.font_weight === 'black' || params.font_weight === 'bold' || params.font_weight === 'regular'
      ? params.font_weight
      : 'regular'
  const animationIn =
    params.animation_in === 'pop' ||
    ...
    params.animation_in === 'fade_in'
      ...
      : 'fade_in'
  const emphasis =
    params.emphasis === 'shake' || params.emphasis === 'flash' || params.emphasis === 'scale_pulse'
      ? params.emphasis
      : undefined
      ...
      ...(colorHintCue
        ? {
            color_label: {
              ...
              square_size_px:
                typeof params.square_size_px === 'number' ? params.square_size_px : 64,
...
function buildOverlayLayer(
  anchor: MigrationProtocolV12['semantic_anchors'][number],
  overlayRecipes: OverlayCapabilityRecipe[] = [],
): OverlayLayer | null {
...
function createEffectFromRecipe(
  preset: string | undefined,
  params: Record<string, unknown> | undefined,
): SceneEffects | undefined {
  if (!preset || !isKnownEffectPreset(preset)) return undefined
...
function isBeatCutDriverRecipe(input: {
  ...
  return (
    input.plugin_id === 'beat_cut_driver' ||
...
function getEnabledBeatCutDriverManifest():
  | NonNullable<ReturnType<typeof getRenderPluginManifest>>
  | undefined {
...
function buildAudioLayers(
  ...
  if (audioAssetId) return [buildBgmAudioLayer(anchor, audioAssetId)]
...
function appendBeatPrimitiveLayers(
  layers: RenderEffectLayer[],
  input: {
    anchor: MigrationProtocolV12['semantic_anchors'][number]
    recipe: MigrationProtocolV12['render_recipe'] | undefined
    beatPlugin: NonNullable<ReturnType<typeof getRenderPluginManifest>>
    idSuffix: string
    source: RenderEffectLayer['source']
    reason: string
    resolution: RenderEffectLayer['resolution']
    beatParams: Record<string, unknown>
  },
): void {
  const pulse = detailPrimitiveEffectForAnchor({
    effect: normalizePrimitiveEffectForAnchor(
      createEffectFromRecipe('primitive_beat_pulse', input.beatParams),
      input.anchor,
    ),
    anchor: input.anchor,
    recipe: input.recipe,
...
const GLOBAL_TEXTURE_PRIMITIVES: SceneEffects['preset'][] = [
...
function expandGlobalEffectPresets(globalEffects: string[] | undefined): SceneEffects['preset'][] {
  ...
  return presets.filter((preset): preset is SceneEffects['preset'] =>
    GLOBAL_TEXTURE_PRIMITIVES.includes(preset as SceneEffects['preset']),
  )
}

function appendGlobalPrimitiveLayers(
  layers: RenderEffectLayer[],
  input: {
    anchor: MigrationProtocolV12['semantic_anchors'][number]
    recipe: MigrationProtocolV12['render_recipe'] | undefined
  ...
  for (const preset of globalPresets) {
    const effect = detailPrimitiveEffectForAnchor({
      effect: normalizePrimitiveEffectForAnchor(
        createEffectFromRecipe(preset, undefined),
        input.anchor,
      ),
      anchor: input.anchor,
      recipe: input.recipe,
...
function buildEffectLayers(input: {
  ...
  resolutions: RenderPlanComponentResolution['decisions']
}): { layers: RenderEffectLayer[]; overlays: OverlayCapabilityRecipe[] } {
  const recipe = input.structure.render_recipe
  const layers: RenderEffectLayer[] = []
  const recipeAnchor =
    (input.anchor as PlanningAnchor).source_anchor_id
      ? {
          ...input.anchor,
          anchor_id: (input.anchor as PlanningAnchor).source_anchor_id!,
        }
      : input.anchor
  ...
  compiled.effects.forEach((sceneRecipe, index) => {
    if (!BEAT_CUT_DRIVER_ENABLED && isBeatCutDriverRecipe(sceneRecipe)) return

    const effect = detailPrimitiveEffectForAnchor({
      effect: normalizePrimitiveEffectForAnchor(
        createEffectFromRecipe(sceneRecipe.preset, sceneRecipe.params),
        input.anchor,
      ),
      anchor: input.anchor,
      recipe,
        ...
        appendBeatPrimitiveLayers(layers, {
          ...
          beatParams: {
            ...
            energy_peaks: (audioDriver.energy_peaks ?? [])
              .filter(
                (peak) =>
                  peak.time >= input.anchor.start_sec && peak.time < input.anchor.end_sec,
              )
              .map((peak) => ({
                ...peak,
            ...
            waveform: (audioDriver.waveform ?? [])
              .filter(
                (sample) =>
                  sample.time >= input.anchor.start_sec && sample.time < input.anchor.end_sec,
              )
              .map((sample) => ({
                ...sample,
  ...
  if (!layers.length) return { layers: [], overlays: compiled.overlays }
...
function selectBackingAudioAsset(input: {
  ...
  if (userAudio) return userAudio.id
  if (input.sampleReference?.use_audio && input.sampleReference.url) return 'sample_reference_audio'
  return undefined
}

function buildSampleReferenceAudioAsset(
  sampleReference: RenderPlanSampleReference | undefined,
): RenderAsset | undefined {
  if (!sampleReference?.use_audio || !sampleReference.url) return undefined
  return {
...
export function buildRenderPlanFromStructure(input: {
  ...
  const assets: RenderAsset[] = materials.map((m) => ({
    id: m.id,
  ...
  if (sampleReferenceAudio) assets.push(sampleReferenceAudio)
  ...
  const componentResolutions: RenderPlanComponentResolution['decisions'] = []

  const scenes: RenderScene[] = planning.anchors.map((anchor, anchorIndex) => {
    ...
    return {
      ...
      visual: buildVisualLayer(
        anchor,
        strategy,
        materials,
        visualReservedMaterialIds,
        anchorIndex,
        forceMaterialMontage,
      ),
