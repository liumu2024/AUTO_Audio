...
function validateRange(input) {
    ...
    if (!finiteNumber(input.start) || !finiteNumber(input.end))
        return;
    if (input.start < 0) {
...
export function validateRemotionTimelineSpec(value) {
    const issues = [];
    if (!isRecord(value)) {
        return {
            ok: false,
    ...
    if (spec.schema_version !== REMOTION_TIMELINE_SPEC_SCHEMA_VERSION) {
        addIssue(issues, 'error', 'schema_version', `schema_version must be ${REMOTION_TIMELINE_SPEC_SCHEMA_VERSION}.`);
    }
    ...
    if (!Array.isArray(spec.assets))
        addIssue(issues, 'error', 'assets', 'assets must be an array.');
    ...
    const briefImageReferenceIds = new Set((Array.isArray(spec.creative_brief?.image_references) ? spec.creative_brief.image_references : [])
    ...
    if (!Array.isArray(spec.scenes))
        addIssue(issues, 'error', 'scenes', 'scenes must be an array.');
    if (scenes.length === 0)
        addIssue(issues, 'error', 'scenes', 'at least one scene is required.');
    const plannedOutputAssetBySceneId = new Map((Array.isArray(spec.material_jobs) ? spec.material_jobs : [])
        .filter((job) => job.output_asset_id)
        .map((job) => [job.scene_id, job.output_asset_id]));
    ...
    scenes.forEach((scene, index) => {
        ...
        if (['user_video', 'ai_video', 'image_motion'].includes(scene.type) && !scene.asset_id) {
            ...
            if (scene.type === 'ai_video' && plannedOutputAssetId) {
                addIssue(issues, 'warning', `${path}.asset_id`, `asset_id will be assigned from planned generated asset ${plannedOutputAssetId} during material resolution.`);
            }
            else {
        ...
        if (scene.asset_id && !assetIds.has(scene.asset_id)) {
            if (plannedOutputAssetBySceneId.get(scene.id) === scene.asset_id) {
                addIssue(issues, 'warning', `${path}.asset_id`, 'asset_id points to a planned generated asset that must be resolved before render.');
            }
            else {
    ...
    sortedScenes.forEach((scene, index) => {
        ...
        if (sceneEnd > next.start_sec + 0.05) {
            addIssue(issues, 'warning', 'scenes', `scene ${scene.id} overlaps with ${next.id}; TransitionSeries will use declared order, not absolute overlap.`);
        }
    });
    ...
    materialJobs.forEach((job, index) => {
        ...
        if (job.input_asset_id) {
            ...
            if (spec.creative_brief && !briefImageReferenceIds.has(job.input_asset_id)) {
                addIssue(issues, 'error', `creative_brief.image_references`, `image-conditioned generation must describe source asset ${job.input_asset_id}.`);
            }
        }
        ...
        if (materialJobMissingRequiredOutput(job, assetIds)) {
            addIssue(issues, 'error', `${path}.output_asset_id`, `${job.type} ${job.status} jobs require an available output asset.`);
        }
...
export function assertValidRemotionTimelineSpec(value) {
...
//# sourceMappingURL=remotion-timeline-validator.js.map