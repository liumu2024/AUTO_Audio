...
export function isEffectSharedParamRef(value) {
    return isRecord(value) && typeof value.$shared === 'string';
}
export function isEffectFollowRef(value) {
    return isRecord(value) && isRecord(value.$follow) && typeof value.$follow.sourceLayerId === 'string';
}
function containsEffectBindingRef(value) {
    if (isEffectSharedParamRef(value) || isEffectFollowRef(value))
        return true;
    if (Array.isArray(value))
        return value.some(containsEffectBindingRef);
    if (!isRecord(value))
        return false;
    return Object.values(value).some(containsEffectBindingRef);
}
export function atomIdFromLayerId(layerId) {
    const parts = layerId.split(':');
    return parts.length >= 2 ? parts[parts.length - 1] : null;
}
function normalizeArrayIndexPath(path) {
    return path
        .replace(/\.(\d+)\./g, '[].')
        .replace(/\.(\d+)$/, '[]')
        .replace(/^(\w+)\[(\d+)\]\./, '$1[].');
}
export function resolveBindingPath(path) {
...
function deleteAtPath(root, path) {
    ...
    if (arrayItemMatch) {
        ...
        if (!Array.isArray(array) || array.length === 0 || !isRecord(array[0]))
            return;
        if (!restPath.includes('.')) {
...
function layerIndexByAtomId(layers) {
    const map = new Map();
    for (const layer of layers) {
        const atomId = atomIdFromLayerId(layer.id);
        if (atomId)
            map.set(atomId, layer);
    }
    return map;
}
...
export function interpolatePathAtTime(keyframes, timeSec) {
...
export function interpolateRadiusAtTime(keyframes, timeSec) {
    if (keyframes.length === 0)
        return 0;
    if (timeSec <= keyframes[0].time)
...
function isPathKeyframes(value) {
...
function isRadiusKeyframes(value) {
    return (Array.isArray(value) &&
        value.length > 0 &&
        isRecord(value[0]) &&
        typeof value[0].time === 'number' &&
...
function parseBeatSyncIndex(sync) {
    const match = /^strong_beat:(\d+)$/.exec(sync.trim());
    if (!match)
        return null;
    return Number.parseInt(match[1], 10);
}
function applySharedGeometry(layers, sharedGeometry) {
    const origin = sharedGeometry?.origin;
    if (!origin)
        return layers;
    return layers.map((layer) => {
        const effects = structuredClone(layer.effects);
        ...
        return { ...layer, effects: effects };
    });
}
function applySharedTimeline(layers, sharedTimeline, beatTimes, warnings, sceneId) {
    if (!sharedTimeline?.sync_points?.length)
        return layers;
    return layers.map((layer) => {
        const effects = structuredClone(layer.effects);
        const revealEvents = effects.reveal_events;
        if (!Array.isArray(revealEvents) || revealEvents.length === 0)
            return layer;
        const atomId = atomIdFromLayerId(layer.id);
        for (const syncPoint of sharedTimeline.sync_points ?? []) {
            ...
            const triggerTime = beatIndex !== null && beatTimes?.[beatIndex] !== undefined
                ? beatTimes[beatIndex]
                : syncPoint.at_sec;
...
function resolveSharedRefValue(input) {
    ...
    const liveValue = input.sourceLayer
        ...
        : undefined;
    const value = liveValue === undefined || containsEffectBindingRef(liveValue)
        ? input.entry.value
        : liveValue;
...
function walkAndResolveBindings(input) {
    ...
    if (Array.isArray(input.node)) {
        return input.node.map((item, index) => walkAndResolveBindings({
            ...input,
            node: item,
...
export function resolveEffectLayerBindings(input) {
    const warnings = [];
    const runtimeFollows = [];
...
export function sampleMaskCenterAtTime(effects, timeSec) {
    const mask = getAtPath(effects, 'mask.position_keyframes');
    if (!isPathKeyframes(mask))
        return null;
...
//# sourceMappingURL=effect-layer-binding-resolver.js.map