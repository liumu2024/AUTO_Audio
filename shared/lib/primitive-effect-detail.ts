...
function anchorEvidenceText(anchor: MigrationProtocolV12['semantic_anchors'][number]): string {
  return [
...
function scenicCueFromText(text: string): { label: string; color: string } {
  if (/red|sunset|rose|夕阳|日落|红/.test(text)) return { label: '红', color: '#ef4444' }
  if (/green|grass|forest|meadow|草|森林|绿/.test(text)) return { label: '绿', color: '#22c55e' }
  if (/blue|sea|lake|water|sky|ocean|海|湖|蓝/.test(text)) return { label: '蓝', color: '#38bdf8' }
  if (/snow|ice|white|雪|冰|白/.test(text)) return { label: '白', color: '#e5e7eb' }
  if (/yellow|gold|warm|金|黄/.test(text)) return { label: '黄', color: '#facc15' }
  return { label: '色', color: '#22d3ee' }
}

function timedPoint(time: number, x_pct: number, y_pct: number, easing?: string, hold?: boolean) {
  return {
    time: Number(time.toFixed(3)),
    x_pct: clampPercent(x_pct),
    y_pct: clampPercent(y_pct),
    ...(easing ? { easing: easing as 'linear' | 'ease-in' | 'ease-out' | 'ease-in-out' | 'overshoot' } : {}),
    ...(hold ? { hold: true } : {}),
  }
}

export function normalizePrimitiveEffectForAnchor(
  ...
  if (effect.preset === 'primitive_mask_reveal') {
    const portalShape =
      typeof record.portal_shape === 'string' ? record.portal_shape : undefined
    return {
      ...effect,
      mask: {
        ...effect.mask,
  ...
  if (effect.preset === 'primitive_texture_grade') {
    const vignetteStrength =
      typeof record.vignette_strength === 'number' ? record.vignette_strength : undefined
...
export function detailPrimitiveOrbMotion(input: {
  ...
  const isContinuous =
    /continuous|always.?moving|nonstop|rapid|fast.?move|quick.?jump|moving.?orb|moving.?ball|一直|持续|快速|小球|光球/.test(
      text,
    )
  ...
  const holdTime = Math.min(
    duration * 0.42,
    firstBeat !== undefined ? Math.max(entryTime + 0.08, firstBeat) : entryTime + (isOpen ? 0.28 : 0.14),
  )
  ...
  const path = isContinuous
    ? [
        ...
        ...pointPalette.concat(pointPalette.slice(0, 2)).map((point, index, points) =>
          timedPoint(
            Math.min(
              duration - 0.04,
              entryTime + (index * Math.max(0.18, duration - entryTime)) / Math.max(1, points.length - 1),
            ),
            point.x,
            point.y,
            index % 2 === 0 ? 'ease-in-out' : 'linear',
          ),
        ),
    ...
    : [
        ...
        ...pointPalette.slice(1).map((point, index) =>
          timedPoint(
            Math.min(duration - 0.08, holdTime + (index + 1) * (duration - holdTime) * 0.28),
            point.x,
            point.y,
            'ease-in-out',
          ),
...
export function detailPrimitiveDirectionalWaveReveal(input: {
  ...
  const isPeak = /peak|outro|final|payoff|closing|burst|color_peak/.test(text)
  const isAccent = /accent|beat|fast|energy/.test(text)
  const wantsPersistentColor =
    /unlock|full.?color|color.?release|accumulate|hold|burst|铺满|全彩|解锁|完全|释放/.test(text)
  const entryTime = Math.min(duration * 0.28, isOpen ? 0.72 : 0.44)
...
export function detailPrimitiveBeatPulse(effect: PrimitiveBeatPulseEffects): PrimitiveBeatPulseEffects {
  return {
    ...effect,
    pulse: {
      scale: 0.055,
...
export function detailPrimitiveTransitionAccentOverlay(input: {
  ...
  const trigger =
    timing.strongBeats[0] ??
    timing.energyPeaks[0]?.time ??
    timing.beats[0]
  const accentDuration = Number(
    Math.min(0.72, Math.max(0.18, effect.duration_sec || duration * 0.14)).toFixed(3),
  )
  const start = isStart
    ? 0
    : trigger !== undefined
...
export function wantsGlobalCinematicGrade(globalEffects: string[] | undefined): boolean {
  return (globalEffects ?? []).some(
    (item) => item === 'primitive_texture_grade' || item === 'cinematic_grade_pack',
  )
}
