import type { EffectRoadmap, EffectRoadmapSegment } from '../types/effect-roadmap.v1.js'
import type { EffectIntent, EffectIntentDocument, EffectIntentId } from '../types/effect-intent.v1.js'
import { EFFECT_INTENT_SCHEMA_VERSION } from '../types/effect-intent.v1.js'

const MOTIF_TO_INTENT: Record<string, EffectIntentId> = {
...
function intentFromSegment(segment: EffectRoadmapSegment): EffectIntent {
  ...
  const unlockMode =
    mustMatch['geometry.reveal_mode'] === 'directional_wave'
      ? 'directional_wave'
  ...
  return {
    ...
    style:
      mustMatch['style.color_transform'] === 'grayscale_to_color'
        ? 'beat_synced_reveal'
        : undefined,
    motion_subject: motionSubject,
    motion_pattern:
      intentId === 'orb_driven_color_wave' ? 'continuous_probe' : undefined,
    unlock_mode: unlockMode,
    reveal_mode:
      typeof mustMatch['geometry.reveal_mode'] === 'string'
        ? mustMatch['geometry.reveal_mode']
        : undefined,
    geometry: Object.fromEntries(
      Object.entries(mustMatch).map(([key, value]) => [
        key.replace(/^geometry\./, ''),
        Array.isArray(value) ? value.map(String) : value,
      ]),
    ) as EffectIntent['geometry'],
    sync: hasBeatSync
...
export function intentIdFromMotifFamily(family: string): EffectIntentId {
