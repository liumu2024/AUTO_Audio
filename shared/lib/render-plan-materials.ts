...
function isProductOrMarketingPrompt(prompt: string): boolean {
...
function isVisualMontagePrompt(prompt: string, visualAssetCount: number): boolean {
  const lower = prompt.toLowerCase()
  const explicitMontage =
...
export function injectMaterialsIntoRenderPlan(input: {
  ...
  if (!visualAssets.length) return input.plan

  const isVisualMontage = isVisualMontagePrompt(prompt, visualAssets.length)
  const existingNonUserAssets = input.plan.assets.filter(
    (asset) =>
      asset.source !== 'user_material' ||
      !input.assets.some((next) => next.id === asset.id),
  )

  const trimCursors = new Map<string, number>()
  const backingAudioAsset =
    audioAsset ?? input.plan.assets.find((asset) => asset.id === 'sample_reference_audio')
