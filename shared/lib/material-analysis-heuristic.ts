import type {
  ...
  MaterialSegment,
  MaterialVisualTag,
} from '../types/material-analysis.js'
...
function normalizeType(
  ...
  return type.toLowerCase() as MaterialAnalysis['type']
}
...
function inferVisualTags(input: HeuristicAssetInput): MaterialVisualTag[] {
  const text = normalizedText(input)
  const type = normalizeType(input.type)
  const tags: MaterialVisualTag[] = [...(input.tags ?? [])]
...
function inferEmotionTags(input: HeuristicAssetInput): MaterialEmotionTag[] {
  const text = normalizedText(input)
  const tags: MaterialEmotionTag[] = []
...
function segmentScore(
  ...
  emotionTags: MaterialEmotionTag[],
): number {
  let score = 0.52
...
export function analyzeMaterialHeuristically(
  input: HeuristicAssetInput,
): MaterialAnalysis {
  const type = normalizeType(input.type)
