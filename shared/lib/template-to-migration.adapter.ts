import {
  isMarketingContentDomain,
  type ContentDomain,
} from '../types/director-grounding.v1.js'
 ...
 */
export function templateToMigrationProtocolV12(
...
function findMaterialForSlot(
  slot: TemplateSchemaV1['slots'][number] | undefined,
  materials: UserMaterialDto[] | undefined,
): UserMaterialDto | undefined {
  if (!slot || !materials?.length) return undefined
  ...
  const acceptedTypes = new Set(
    (slot.accepted_material_types?.length
  ...
  return materials.find((m) => {
    if (!acceptedTypes.has(m.material_type)) return false
    const materialTags = (m.ai_tags ?? []).map((tag) => tag.toLowerCase())
    return materialTags.some((tag) => slotTags.has(tag))
  })
}

function resolveLogicRole(creativeRole: string, contentDomain?: ContentDomain): string {
...
function mapSegmentNameToRole(name: string): string {
  const n = name.toLowerCase()
  if (n.includes('hook')) return 'hook'
  if (n.includes('cta')) return 'cta'
  if (n.includes('product') || n.includes('demo')) return 'product_demo'
  if (n.includes('benefit') || n.includes('pain')) return 'pain_amplify'
  if (n.includes('open') || n.includes('intro')) return 'cinematic_open'
  if (n.includes('groove') || n.includes('montage')) return 'groove_montage'
  if (n.includes('accent') || n.includes('beat')) return 'accent_beat_sequence'
  if (n.includes('peak') || n.includes('outro') || n.includes('closing')) return 'color_peak_outro'
  return n.replace(/[^a-z0-9_ -]+/g, '').trim().replace(/[\s-]+/g, '_') || 'creative_segment'
}

function buildVisualPrompt(
  seg: TemplateSchemaV1['structure'][number],
  slot: TemplateSchemaV1['slots'][number] | undefined,
): string {
  const parts = [seg.purpose]
  if (seg.camera) parts.push(`镜头：${seg.camera}`)
  if (seg.motion) parts.push(`运镜：${seg.motion}`)
  if (slot?.tags.length) parts.push(`素材标签：${slot.tags.join('、')}`)
  return parts.join('；')
}
