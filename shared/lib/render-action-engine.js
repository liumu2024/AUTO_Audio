...
function applyBindMaterial(plan, payload) {
    return {
        ...plan,
...
function applyEffectLayer(plan, payload) {
...
function applyAddOverlay(plan, payload) {
    return {
        ...plan,
        scenes: plan.scenes.map((scene) => {
            if (scene.id !== payload.sceneId)
...
function applySetAudio(plan, payload) {
    return {
        ...plan,
        scenes: plan.scenes.map((scene) => {
            if (scene.id !== payload.sceneId)
            ...
            return {
                ...
                audio: scene.audio.map((layer) => {
                    if (payload.audioId && layer.id !== payload.audioId)
                        return layer;
...
function applySetCanvas(plan, payload) {
    if (!payload.aspectRatio)
        return plan;
    return applyAspectRatioToRenderPlan(plan, payload.aspectRatio);
}
function applyRequestComponent(plan, payload) {
...
export function applyRenderAction(plan, action) {
    const next = clonePlan(plan);
    switch (action.type) {
        case 'BIND_MATERIAL':
            return applyBindMaterial(next, action.payload);
        case 'APPLY_EFFECT_LAYER':
            return applyEffectLayer(next, action.payload);
        case 'ADD_OVERLAY':
            return applyAddOverlay(next, action.payload);
        case 'SET_AUDIO':
            return applySetAudio(next, action.payload);
        case 'SET_CANVAS':
            return applySetCanvas(next, action.payload);
        case 'REQUEST_COMPONENT':
            return applyRequestComponent(next, action.payload);
        default:
            return next;
    }
}
export function applyRenderActions(plan, actions) {
    return actions.reduce((current, action) => applyRenderAction(current, action), plan);
}
export function applyRenderActionBatch(plan, batch) {
    return applyRenderActions(plan, batch.actions);
}
/** 将 REVISE_RENDER_PLAN 的 slots 变更翻译为 RenderAction 列表 */
export function renderActionsFromSlotsPatch(slots, plan) {
    const actions = [];
...
export function sceneEffectFromPreset(preset) {
    if (!isKnownEffectPreset(preset))
        return undefined;
    return createDefaultEffect(preset);
}
//# sourceMappingURL=render-action-engine.js.map