...
const SUPPORTED_EFFECT_PRESETS = new Set<string>([
...
function validateAssetRef(
  ...
  input: {
    assetId: unknown
    path: string
    assetsById: Map<string, RenderAsset>
    required?: boolean
  },
) {
...
function validateEffectPreset(
  issues: RenderPlanValidationIssue[],
  effects: SceneEffects | undefined,
  path: string,
) {
  if (!effects) return
  const preset = (effects as { preset?: unknown }).preset
  ...
  if (preset === 'generated_component') {
    const componentId = (effects as { component_id?: unknown }).component_id
    const fallbackPreset = (effects as { fallback_preset?: unknown }).fallback_preset
...
function toToolError(issue: RenderPlanValidationIssue): DirectorToolError {
  return {
    code: issue.code,
...
export function validateRenderPlanHard(
  input: RenderPlanValidationInput,
): DirectorToolResult<RenderPlanValidationReport> {
  const phase = input.phase ?? 'before_render'
  const issues: RenderPlanValidationIssue[] = []
  ...
  for (const [index, asset] of assets.entries()) {
    ...
    validateAsset(
      ...
      index,
      phase,
      input.allowPlaceholderUrls ?? phase === 'before_save',
    )
  }
  ...
  const report: RenderPlanValidationReport = {
    phase,
...
export function formatRenderPlanValidationFailure(
  result: DirectorToolResult<RenderPlanValidationReport>,
): string {
  const first = result.errors[0]
  if (!first) return 'PLAN_SCHEMA_INVALID: RenderPlan validation failed'
  return `${first.code}: ${first.message}`
}

export function assertRenderPlanValid(
  input: RenderPlanValidationInput,
): RenderPlanValidationReport {
