...
function classifyConversationIntent(text, slots) {
    ...
    if (Object.keys(slotsPatch).length > 0)
        return { intent: 'revise_plan', confidence: 0.7 };
...
export function routeDirectorConversation(input) {
    ...
    if (wantsOutput) {
        const missingSlots = [];
        if (mergedSlots.sampleVideoStatus !== 'parsed')
            missingSlots.push('sampleVideoStatus');
        if (!input.runtime.hasVisualMaterial)
            missingSlots.push('materialStatus');
        if (!input.runtime.hasPipeline || !input.runtime.activeTaskId)
            missingSlots.push('activeTask');
...
function motifTextFromTemplate(template) {
    const structure = template?.structure ?? [];
    return [
        JSON.stringify(template?.style_features ?? {}),
        JSON.stringify(template?.transitions ?? []),
        ...structure.map((segment) => [segment.purpose, segment.camera, segment.motion, segment.subtitle]
            .filter(Boolean)
            .join(' ')),
    ]
        .join(' ')
...
function motifsFromText(motifText) {
    const motifs = [];
    if (/grayscale|black.?white|黑白|去色/.test(motifText))
        motifs.push('grayscale_base');
    if (/portal|光环|圆环|局部彩色/.test(motifText))
        motifs.push('color_portal');
    if (/ripple|wave|波纹|水波|涟漪/.test(motifText))
        motifs.push('ripple_displacement');
    if (/sweep|cinematic|扫光|电影感/.test(motifText))
        motifs.push('cinematic_light_sweep');
    if (/split|collage|分屏|拼贴/.test(motifText))
        motifs.push('editorial_collage');
    return motifs.length ? motifs : ['template_structure', 'material_montage'];
}
function presetsFromMotifs(motifs) {
    const presets = [
...
export function buildSampleStyleRecipe(template) {
    const structure = template?.structure ?? [];
    ...
    return {
        ...
        pacing: template?.style_features?.pace === 'fast'
            ? 'fast_cut'
        ...
        timeline_pattern: structure.length
            ? structure.map((segment, index) => ({
                ...
                transition_to_next: template?.transitions.find((transition) => transition.from_segment_id === segment.id)?.presentation ?? undefined,
            }))
            : [
                {
                    phase: '开场氛围',
...
export function buildSampleStyleRecipeFromMigration(structure) {
    ...
    return {
        ...
        timeline_pattern: anchors.map((anchor, index) => ({
            phase: anchor.logic_intent.creative_role ??
                anchor.logic_intent.marketing_role ??
                `段落_${index + 1}`,
...
function segmentFromAssetAnalysis(analysis) {
...
export function buildMaterialAnalysis(material) {
    ...
    return {
        ...
        type,
        usable_segments: segments.length > 0
            ? segments
            : [
                {
                    start_sec: 0,
