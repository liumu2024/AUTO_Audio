...
const commands: Array<{ id: string; script: string; args?: string[]; tsconfig?: string }> = [
  ...
  { id: 'director_turn_idempotency', script: 'scripts/smoke-v2-director-turn-idempotency.ts' },
  ...
  { id: 'agent_tool_idempotency', script: 'scripts/smoke-v2-agent-tool-idempotency.ts' },
  ...
  { id: 'creative_knowledge', script: 'scripts/smoke-v2-creative-knowledge.ts' },
...
const markdown = [
  ...
  `- 用户偏好决策样本：${preparedMemory.samples}（holdout ${preparedMemory.holdoutCases * 5} 轮）`,
  `- 产物要求场景：20`,
  ...
  '## 普适创作知识（CreativeKnowledge）', '',
  `- 候选隔离、人工采纳、active Top-K 与无关项不补齐：${commandResults.find((item) => item.id === 'creative_knowledge')?.ok ? '通过' : '失败或未运行'}`, '',
  '## 硬门禁', '',
