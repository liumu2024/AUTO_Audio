...
状态：既有 P0 主链已实现；2026-08-12 真实评测发现 AI 镜头复用的最终文件哈希未闭环，列为待修 P0；P1/P2 与 revision fragment 按门槛延期
...
- 新的字幕或转场逐对象 Tool 协议。
- 创作偏好的语义检索或重排升级；当前继续使用既有 BM25/关键词检索，独立实施条件与最小升级顺序见第 12 节。
...
- 新 RenderRun 已具备按 Provider 请求指纹、文件 SHA256 和 ffprobe 可读性复用上一成功 Run 的机制；但当前生成 Trace 保存的哈希与最终二次标准化文件不一致，真实字幕二次渲染未命中复用。修复后仍要求新 Run 自包含。
...
该项不进入当前 P0/P1。现有冻结评测已经暴露“相关记录能召回，但会混入共享表面词的无关记录”：`Active Recall@8` 为 100%，跨草稿错误召回为 0，但 25 个查询中累计出现 9 条无关召回。当前实际用户库规模仍小，暂不足以证明需要引入向量检索、Reranker 或第二次模型调用，因此先保留既有中文二元词、BM25、连续短语加分和 Top-K 链路。

重新启动该里程碑前，需要同时具备：

- 有代表性的真实偏好规模和查询分布，或连续出现用户可感知的误召回；
- 扩充后的冻结检索集继续覆盖 user/draft scope、active/candidate/revoked、跨草稿、无关查询和近义干扰；
- 当前 BM25 的 Recall、nDCG、无关召回、延迟和结果数量基线已冻结。

实施顺序必须从小到大：

1. 先在现有 BM25 结果上增加相对 Top-1 分数截断或同类最小差距，只过滤明显弱相关结果；
2. 若无法在保持 Recall 和跨草稿隔离的同时降低无关召回，再评估 Query 改写、向量召回或轻量重排；
3. 不因未来可能扩库而预建向量数据库，不让检索升级改变 CreativeMemory 的作用域、状态和 receipt 语义。

合并门槛：扩充集上的 active recall 不低于旧基线，跨草稿错误召回仍为 0，无关召回显著下降；线上延迟和 Director 输入规模不得明显恶化。该里程碑与 revision fragment 可放在同一未来规划文档中跟踪，但必须独立实施、独立提交和独立回滚，不能把检索协议与 Timeline revision 协议绑成一次大改。
